###Function to get entire XTheta matrix-------------------------------------------------------------------
GetXTheta <- function(Theta, XThetaInd, TimeVec, OneNu, OneN, tNu, N, M) {
  ThetaLong <- kronecker(OneNu, Theta)
  Stacks <- matrix(1, nrow = N, ncol = 2)
  Stacks[, 2] <- (TimeVec - ThetaLong) * (1 * (ThetaLong <= TimeVec))
  XTheta <- matrix(0, nrow = N, ncol = 2 * M)
