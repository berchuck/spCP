...
arma::mat SamplePPD(Rcpp::List DatObj_List, Rcpp::List Para_List, int NKeep) {
  ...
  for (int s = 0; s < NKeep; s++) {
    ...
    Eta = EtaMat.col(s);
    Theta = arma::min(tNu * OneM, arma::exp(Eta));
...
arma::mat SamplePPD_lmc(Rcpp::List DatObj_List, Rcpp::List Para_List, int NKeep) {
  ...
  //Initialize objects
  arma::colvec YStar(N), Y(N), Theta(M), Mu(N), Sigma2(N);
  arma::colvec Beta0(M), Beta1(M), Lambda0(M), Lambda1(M);
  arma::colvec Eta(M), YMax(N, arma::fill::zeros);;
  arma::mat XTheta(N, M), OmegaChol(N, N);
  arma::mat PPD(N, NKeep);
  arma::umat ProbitOnes;
  ...
  for (int s = 0; s < NKeep; s++) {
    ...
    //Compute moments and get log-likelihood
    Beta0 = Beta0Mat.col(s);
    Beta1 = Beta1Mat.col(s);
    Lambda0 = Lambda0Mat.col(s);
    Lambda1 = Lambda1Mat.col(s);
    Eta = EtaMat.col(s);
    Theta = arma::min(tNu * OneM, arma::exp(Eta));
    XTheta = GetXTheta_lmc(Theta, XThetaInd, TimeVec, OneNu, OneN, tNu, N, M);
    Mu = arma::kron(OneNu, Beta0) + XTheta * Beta1;
    Sigma2 = arma::exp(2 * (arma::kron(OneNu, Lambda0) + XTheta * Lambda1));
...
arma::mat SamplePPD_novar(Rcpp::List DatObj_List, Rcpp::List Para_List, int NKeep) {
  ...
  arma::mat Beta0Mat = Para.Beta0;
  arma::mat Beta1Mat = Para.Beta1;
  arma::mat LambdaMat = arma::trans(Para.Lambda0);
  arma::mat EtaMat = arma::trans(Para.Eta);
  ...
  //Initialize objects
  arma::colvec YStar(N), Y(N), Theta(M), Mu(N), Sigma2Loc(M), Sigma2(N);
  arma::colvec Lambda(M);
  arma::rowvec Beta0(M), Beta1(M);
  arma::colvec Eta(M), YMax(N, arma::fill::zeros), Beta(2 * M);
  arma::mat XTheta(N, M), OmegaChol(N, N), BetaMat(2, M);
  arma::mat PPD(N, NKeep);
  arma::umat ProbitOnes;
  ...
  for (int s = 0; s < NKeep; s++) {
    ...
    BetaMat.row(1) = Beta1;
    Beta = arma::vectorise(BetaMat);
    Lambda = LambdaMat.col(s);
    Eta = EtaMat.col(s);
    Theta = arma::min(tNu * OneM, arma::exp(Eta));
    XTheta = GetXTheta(Theta, XThetaInd, TimeVec, OneNu, OneN, tNu, N, M);
    Mu = XTheta * Beta;
    Sigma2Loc = arma::exp(2 * Lambda);

    //Sample from posterior predictive distribution
    YStar = rmvnormRcpp(1, Mu, arma::kron(EyeNu, arma::diagmat(Sigma2Loc)));
