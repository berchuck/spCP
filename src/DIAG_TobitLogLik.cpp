...
arma::colvec TobitLogLik(datobjDIAG DatObj, paraDIAG Para, int NKeep) {
  ...
  for (int s = 0; s < NKeep; s++) {
    ...
    for (int i = 0; i < N; i++) {
      Y = YObserved(i);
      if (Y > 0) loglik += dlnorm(Y, Mu(i), Sigma2(i));
...
double TobitLogLikMean(datobjDIAG DatObj, paraDIAG Para) {
  ...
  for (int i = 0; i < N; i++) {
    Y = YObserved(i);
    if (Y > 0) LogLik += dlnorm(Y, MuMean(i), Sigma2Mean(i));
...
arma::colvec TobitLogLik_novar(datobjDIAG DatObj, paraDIAG Para, int NKeep) {
  ...
  //Initialize objects
  arma::colvec LogLik(NKeep), Theta(M), Mu(N), Sigma2(N), Sigma2Loc(M);
  arma::rowvec Beta0(M), Beta1(M);
  arma::colvec Eta(M), Beta(2 * M), Lambda(M);
  arma::mat XTheta(N, 2 * M), BetaMat(2, M), OmegaChol(N, N);
  double loglik, Y;
  Rcpp::Rcout << std::fixed << "Calculating Log-Lik: 0%.. ";
  ...
  for (int s = 0; s < NKeep; s++) {
    ...
    Sigma2 = arma::kron(OneNu, Sigma2Loc);
